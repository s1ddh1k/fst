...
test("auto research orchestrator iterates, writes artifacts, and promotes best candidate", async () => {
  ...
  const savedSession = await readFile(path.join(outputDir, "session.md"), "utf8");
  ...
  assert.match(savedSession, /## Objective/);
  assert.match(savedSession, /## Promotion Gate/);
  assert.match(savedSession, /relative-momentum-pullback/);
});
...
test("auto research final keep_searching preserves the review decision while keeping the top evaluation", async () => {
  ...
  assert.equal(review?.promotedCandidateId, undefined);
  assert.equal(report.bestCandidate?.candidate.candidateId, "final-winner");
  assert.doesNotMatch(review?.observations.join("\n") ?? "", /Final iteration kept searching/i);
});

test("auto research keeps the LLM promotion choice instead of replacing it with governance", async () => {
  ...
  assert.equal(review?.promotedCandidateId, "fragile-candidate");
  assert.equal(report.bestCandidate?.candidate.candidateId, "fragile-candidate");
...
test("auto research dedupes duplicate keep_searching candidates without inventing extra ones", async () => {
  ...
  assert.equal(report.iterations[1]?.evaluations.length, 1);
  assert.ok(
